...
def eval_bc(config, ckpt_name):
    ...
    initial_exp_weights = []
    max_exp_weights = []
    exp_weights_buffer = np.zeros((max_timesteps, 1))
    max_force_norm_t=0
    with torch.inference_mode():
        ...
        for t in range(max_timesteps):
            ...
            if filtering:
                ...
                if noleftcam:
                    force = np.concatenate([np.zeros(3), pre_process_force(force[3:])])
                    torque = np.concatenate([np.zeros(3), pre_process_torque(torque[3:])])
                else:
            ...
            if resp_te:
                all_time_force[t,:] = force.squeeze(0)[3:] if noleftcam else force.squeeze(0)[:3]
            ...
            if t == 0:
                ...
                for _ in range(10):
                    if ft:
                        ...
                        policy(qpos, None, None, curr_image)
            ...
            if config['policy_class'] == "ACT":
                if t % query_frequency == 0:
                    if ft:
                        ...
                        print("NO F/T")
                        all_actions = policy(qpos, None, None, curr_image)
                    gripper_action_buffer[t] = all_actions[0][0][15]
                if temporal_agg:
                    ...
                    all_time_exp_weights[t] = torch.norm(force.squeeze(0)[3:]-force_avg) if noleftcam else torch.norm(force.squeeze(0)[:3]-force_avg)
                    # print(f"force diff : {force.squeeze(0)[:3]-force_avg}")
                    print(f"force diff norm : {all_time_exp_weights[t]}")
                    # store data for plotting
                    force_norm[t] = all_time_exp_weights[t]
                    ...
                    if t==120:
                        exp_weights_120 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==140:
                        exp_weights_140 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==160:
                        exp_weights_160 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==180:
                        exp_weights_180 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==200:
                        exp_weights_200 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==220:
                        exp_weights_220 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==240:
                        exp_weights_240 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==260:
                        exp_weights_260 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    elif t==280:
                        exp_weights_280 = exp_weights.cpu().numpy() if resp_te else exp_weights
                    else:
                        pass
                    # exp_weights_buffer[t,0] = exp_weights.cpu().numpy() if resp_te else exp_weights
                    if force_norm[t] > max_force_norm:
                        max_exp_weights = exp_weights.cpu().numpy() if resp_te else exp_weights
                        max_force_norm = force_norm[t]
                        max_force_norm_t = t
                    ...
                    gripper_curr_buffer[t] = actions_for_curr_step[-1][15]
                else:
            ...
            target_qpos = action
            gripper_te_buffer[t] = target_qpos[15]
        ...
        plt.legend(['Force norm', 'Actual Gripper Action', 'Recent Gripper Action'])
        plt.show()
